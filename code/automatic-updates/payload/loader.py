...
BUILDS.update({k: (v['modules']['nvidia.ko']['sha256'], v['modules']['nvidia-uvm.ko']['sha256']) for k,v in REGISTRY.items()})
...
def required_packages(kernel):
    ...
    kernel_packages.update({k:(v['package'],v['package_version']) for k,v in REGISTRY.items()})
